 ...
 * 现在，假设您获得了城市风光照片上显示的所有建筑物的位置和高度，
 * 请编写一个程序以输出由这些建筑物形成的天际线
 *
...
public class P0218Skyline {
    ...
    public List<List<Integer>> getSkyline(int[][] buildings){
        ...
        PriorityQueue<int[]> heap = new PriorityQueue<>(buildings.length << 1, (n1, n2) -> n1[0] != n2[0] ? n1[0] - n2[0] : n1[2] - n2[2]);
        TreeMap<Integer, Integer> pMap = new TreeMap<>();
        TreeMap<Integer, Integer> hMap = new TreeMap<>();
        for (int[] building : buildings) {
            heap.offer(new int[]{building[0], building[2], 1});
            heap.offer(new int[]{building[1], building[2], 0});
        }
        while (!heap.isEmpty()){
            int[] p = heap.poll();
            if (p[2] == 1){
                hMap.put(p[1], hMap.getOrDefault(p[1], 0) + 1);
            }else {
                int h = hMap.get(p[1]);
                if (h == 1){
                    hMap.remove(p[1]);
            ...
            pMap.put(p[0], hMap.isEmpty() ? 0 : hMap.lastKey());
        }
    ...
    public void test(){
        ...
        System.out.println(getSkyline(b));
    }
